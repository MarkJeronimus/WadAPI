...
public final class ExtendedNodesCodec {
	...
	public static void decode(FileBufferLump lump, WadMapBuilder builder) {
		...
		decodeSubsectorPart(lump, builder);     // Subsectors are only stored in the NODES lump.
		decodeSegmentsPart(lump, builder);      // Segments   are only stored in the NODES lump.
		decodeNodesPart(lump, builder);         // Nodes      are only stored in the NODES lump.
	}
	...
	private static void decodeVerticesPart(FileBufferLump lump, VerticesLump verticesLump) {
		...
		if (numNewVertices < 0 || numNewVertices > maxNumVertices)
			throw new IllegalArgumentException("Corrupt or invalid extended " + lumpName + " lump. " +
			                                   "Number of new vertices incorrect: " + numNewVertices +
			                                   " (offset " + (fileBuffer.position() - 4) + ')');
	...
	private static void decodeSubsectorPart(FileBufferLump lump, WadMapBuilder builder) {
		String     lumpName   = lump.getName();
		FileBuffer fileBuffer = lump.getFileBuffer();
		...
		if (numSubsectors < 0 || numSubsectors > maxNumSubsectors)
			throw new IllegalArgumentException("Corrupt or invalid extended " + lumpName + " lump. " +
			                                   "Number of subsectors incorrect: " + numSubsectors +
			                                   " (offset " + (fileBuffer.position() - 4) + ')');
	...
	private static void decodeSegmentsPart(FileBufferLump lump, WadMapBuilder builder) {
		String     lumpName   = lump.getName();
		FileBuffer fileBuffer = lump.getFileBuffer();
		...
		if (numSegments < 0 || numSegments > maxNumSegments)
			throw new IllegalArgumentException("Corrupt or invalid extended " + lumpName + " lump. " +
			                                   "Number of segments incorrect: " + numSegments +
			                                   " (offset " + (fileBuffer.position() - 4) + ')');
	...
	private static void decodeNodesPart(FileBufferLump lump, WadMapBuilder builder) {
		String     lumpName   = lump.getName();
		FileBuffer fileBuffer = lump.getFileBuffer();
		...
		if (numNodes < 0 || numNodes > numNodesCalculated)
			throw new IllegalArgumentException("Corrupt or invalid extended " + lumpName + " lump. " +
			                                   "Number of nodes incorrect: " + numNodes +
			                                   " (offset " + (fileBuffer.position() - 4) + ')');
